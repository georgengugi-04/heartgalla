...
- **First-visit intro** — typographic sequence with synthesised music (`components/intro/`). How often it plays is one
  line: `INTRO_FREQUENCY` in `components/intro/introGuard.ts` (`"session"` = every new visit, the default; `"always"`;
...
- **Art Lab, in the live order** (`app/art-lab/page.tsx`): 04 The Collector's Eye · 05 Art Without Borders · 06 The Living
  Canvas · 07 The Sound of Colour · 08 Curate Your Wall · 09 Art Alchemy · 10 The Art Dialogue. Artwork lists for the newer
  ones are in `lib/lab.ts`; the Art Dialogue's observations are in `lib/annotations.ts`.
