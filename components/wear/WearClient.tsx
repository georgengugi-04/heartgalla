...
export default function WearClient() {
            ...
            <div className="flex gap-3 flex-wrap">
              {CHOICES.map((a) => (
                <button
                  ...
                  aria-label={`Preview with ${a.title}`}
                  aria-pressed={artwork.id === a.id}
                  className={`w-16 h-16 rounded-lg overflow-hidden border-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-gold ${artwork.id === a.id ? "border-gold" : "border-transparent"}`}
                >
            ...
            <div className="flex gap-3 flex-wrap">
              {GARMENTS.map((g) => (
                <button
                  ...
                  className={`label-mono border rounded-full px-4 py-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-gold ${garment === g ? "border-gold text-gold" : "border-ivory/30"}`}
                >
                  {g}
                </button>
              ))}
            ...
            <div className="flex gap-3 flex-wrap">
              {PLACEMENTS.map((p) => (
                <button
                  ...
                  className={`label-mono border rounded-full px-4 py-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-gold ${placement === p ? "border-gold text-gold" : "border-ivory/30"}`}
                >
                  {p}
                </button>
              ))}
